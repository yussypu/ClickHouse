...
# Concurrent ALTER against a running INSERT ... SELECT under async_insert. Split out of
# 04633_async_insert_select_regression, which reached three minutes by holding every race of this
# feature: each case has to keep an insert in flight for seconds, so the only way to shorten them is
# to run them as separate tests. Case numbers are kept from that test.
#
# Every insert sleeps per row, so the ALTER lands while the SELECT is still reading, which is what
# pins that the pipeline keeps using the schema frozen before the SELECT started.
