...
# An INSERT ... SELECT under async_insert resolves both the destination schema and the dependency
# graph before the SELECT starts, and a DDL query arriving mid-flight must not change either. Split
# out of 04633_async_insert_select_regression, which reached three minutes by holding every race of
# this feature in one file. Case numbers are kept from that test.
