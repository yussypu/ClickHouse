...
{
public:
    /// `report_read_progress` defaults to false: callers whose reads are not counted elsewhere
    /// pass true explicitly, so a new caller does not double count reads by accident.
    WaitForAsyncInsertSource(
        std::future<AsyncInsertProgress> insert_future_,
        size_t timeout_ms_,
        QueryStatusPtr process_list_elem_,
        ProgressCallback progress_callback_,
        bool report_read_progress_ = false)
