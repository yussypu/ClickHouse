...
/// Granularity of the cancellation check during the flush wait; `wait_for` returns as soon as the
/// future is ready, so this adds no latency of its own.
...
size_t estimateMaterializedBytes(const ColumnPtr & column, size_t rows)
{
    ...
    {
        /// The fan-out is not uniform, so price each nested row by how many rows reference it: an
        /// average undershoots when one large row is referenced thousands of times. Counting beats
        /// `byteSizeAt` per output row, which rescans a whole nested array on every reference.
    ...
    /// Unlike const, both terms are needed: `byteSize` prices the stored non-default values and
    /// `byteSizeAt(0)` the default that each expanded row costs.
    if (column->isSparse())
    {
...
AsyncInsertQueueTransform::GenerateResult AsyncInsertQueueTransform::getRemaining()
{
    if (held)
    {
        ...
        /// `wait_for_async_insert` is deliberately not honoured: returning early would hide a flush
        /// failure from a client already told the INSERT succeeded. Cancelling stops the wait, not the
        /// write.
        ...
        while (true)
        {
            ...
            {
                /// Called for its throwing side effect only: a kill, or `max_execution_time` under
                /// `timeout_overflow_mode = 'throw'`. Its `false` return under `'break'` is ignored,
                /// because the block is already queued and returning here would report success to a
                /// client whose flush can still fail.
        ...
        /// The future is already ready here, so the timeout below is never waited on, only named.
        /// `report_read_progress=false`: the `SELECT` side already reported its own reads.
