...
ColumnsDescription QueryLogElement::getColumnsDescription()
{
    ...
    return ColumnsDescription
    {
        ...
        {"written_rows", std::make_shared<DataTypeUInt64>(), "The number of rows written by the query, including any rows written by downstream inserts triggered by the pipeline, such as attached materialized views. For a synchronous insert these downstream rows are recorded on the `query_kind` = `Insert` entry; for an asynchronous insert they are recorded on the `query_kind` = `AsyncInsertFlush` entry, while the client-facing `Insert` entry records only the rows accepted from the client, except for an `INSERT ... SELECT` diverted to the asynchronous insert queue as a single block, whose client-facing `Insert` entry waits for the flush and records the actual written rows. For queries that do not write rows, it is 0."},
        {"written_bytes", std::make_shared<DataTypeUInt64>(), "The number of bytes written by the query (uncompressed), including any bytes written by downstream inserts triggered by the pipeline, such as attached materialized views. For a synchronous insert these downstream bytes are recorded on the `query_kind` = `Insert` entry; for an asynchronous insert they are recorded on the `query_kind` = `AsyncInsertFlush` entry, while the client-facing `Insert` entry records only the bytes accepted from the client, except for an `INSERT ... SELECT` diverted to the asynchronous insert queue as a single block, whose client-facing `Insert` entry waits for the flush and records the actual written bytes. For queries that do not write data, it is 0."},
        {"result_rows", std::make_shared<DataTypeUInt64>(), "Number of rows in the result of a SELECT query, or the number of rows written by an insert. For a synchronous insert this includes rows written by downstream inserts triggered by the pipeline (such as attached materialized views) on the `query_kind` = `Insert` entry; for an asynchronous insert those downstream rows are recorded on the `query_kind` = `AsyncInsertFlush` entry, while the client-facing `Insert` entry records only the rows accepted from the client, except for an `INSERT ... SELECT` diverted to the asynchronous insert queue as a single block, whose client-facing `Insert` entry waits for the flush and records the actual written rows."},
