    ...
    ASTInsertQuery & query, StoragePtr table, QueryPipelineBuilder & pipeline, bool add_async_insert_queue_transform)
{
    ...
    /// deduplication setting through `async_insert_deduplicate` instead. Pin the decision whenever
    /// the queue route is possible, so the same query does not deduplicate on one route and skip it
    /// on the other depending on how many blocks the SELECT happened to produce.
