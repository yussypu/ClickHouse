...
/// A `StorageAlias` forwards both reads and writes to its target, so the whole chain is one
/// destination. Returns false when the chain is broken or too deep, which counts as unresolvable.
...
/// True if this SELECT pipeline reads the destination table, or if that cannot be established.
///
/// The queue route makes the SELECT side's share lock outlive the wait for the flush thread, which
/// takes its own lock on the destination; a DDL write request arriving in between would stall both.
/// Every read path registers the storages it keeps alive in the pipeline resources, so ask the built
/// pipeline instead of re-deriving the table set from the AST.
    ...
    ASTInsertQuery & query, StoragePtr table, QueryPipelineBuilder & pipeline, bool add_async_insert_queue_transform)
{
    ...
    /// The queue flush rebuilds the dependency graph with `async_insert=true`, which resolves the
    /// deduplication setting through `async_insert_deduplicate` instead. Pin the decision whenever the
    /// queue route is possible, so both routes deduplicate the same query the same way.
...
BlockIO InterpreterInsertQuery::execute()
{
    ...
    /// This lock does not outlive the flush wait, but the SELECT side's own lock does, which is why the
    /// queue route is refused below for a SELECT that may read the destination.
    ...
    if (query.select && !query.async_insert_flush)
    {
        ...
        if (!res.pipeline.initialized())
        {
            /// The queue cannot serve a transaction, a non-parallel quorum, the `CREATE TABLE ... AS
            /// SELECT` populate, a remote destination, or a table function, which `getTable` would
            /// re-resolve on the flush thread in a different context. The remaining guards need the
            /// built SELECT pipeline, so `addInsertToSelectPipeline` makes the final decision.
            std::string_view reason;
