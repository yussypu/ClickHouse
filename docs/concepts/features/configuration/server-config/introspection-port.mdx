...
An introspection port is a [composable protocol](/operations/settings/composable-protocols) endpoint marked with `<introspection>true</introspection>`. No introspection port is configured by default:
...
- The port accepts `SELECT`, `SHOW`, `DESCRIBE`, `EXPLAIN`, `EXISTS`, `KILL QUERY`, `SYSTEM`, `SET` and `USE`. Statements outside this allow list, including DDL and data-modification queries, are rejected with `QUERY_IS_PROHIBITED`. `SYSTEM` commands are allowed because they are useful for incident response; operators should use the port only from a trusted network.
